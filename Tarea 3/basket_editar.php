...
$bd=new Database();
$id = $_GET['id'];  
$query = "select * from basquetbolistas where id =$id";
...
if (isset($_POST) && !empty($_POST)) {
  ...
  $query = "update basquetbolistas set id='$numero', nombre='$nombre', posicion='$posicion', carrera='$carrera', email='$email' where id=$id";
  if ($bd->operacion($bd->conexion,$query)) {
    //$message="Datos insertados con éxito";
    //$class="alert alert-success";
    $query = "select * from basquetbolistas where id =$numero";
    $res=$bd->selectEspecial($bd->conexion,$query);
    $res = mysqli_fetch_assoc($res);
